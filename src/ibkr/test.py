...
from ipywidgets import interact, IntSlider, Text, Checkbox,Dropdown
...
# ---------------------------
def plot_live_butterfly_legs(lower_strike, middle_strike, upper_strike, expiry, contracts, unbalanced, exchange):
    # Connect to IBKR
    ib = IB()
    ...
    # Create option contracts with the selected exchange
    strikes = [lower_strike, middle_strike, upper_strike]
    option_contracts = [
    Option('SPX',
           lastTradeDateOrContractMonth=expiry[:6],  # YYYYMM
           strike=s,
           right=right,
           exchange=exchange,
           currency='USD',
           multiplier='100',       # required for index options
           tradingClass='SPX')     # disambiguates contract
    for s in strikes
]
...
interact(plot_live_butterfly_legs,
         ...
         exchange=Dropdown(
             options=['CBOE', 'BOX', 'BATS', 'NYSE', 'NASDAQ', 'AMEX', 'ISE', 'C2', 'MIAX', 'PHLX'],
             value='CBOE',
